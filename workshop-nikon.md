...
移行が完了したら、コードをコミットして Pull Request を作成しましょう。
...
Step 2.4 で作成した PR を確認しましょう。以下が自動的に実行されているはずです：

- ✅ Copilot Code Review（自動レビュー）
- ✅ CodeQL セキュリティスキャン

> aside positive
> **ポイント**: 設定前に作成した PR にはこれらのチェックが適用されていません。設定後に新しい PR を作成すると、自動的にチェックが実行されます。
