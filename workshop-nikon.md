...
このワークショップでは、**2008年時代のレガシーウェブサイト（Nikora Corporation）** を題材に、GitHub Copilot の全機能を活用してモダンなウェブアプリケーションに変革する体験をしていただきます。
...
> aside negative
> **⚠️ 重要**: リポジトリ作成時に **Visibility は必ず「Public」** を選択してください。Private リポジトリでは、一部の Copilot 機能や GitHub Actions が正しく動作しない場合があります。
...
1. VS Code のエクスプローラーで `1.nikora-website/index.html` を右クリック
2. **「Open with Live Server」** を選択
3. ブラウザでレガシーサイトが表示されることを確認

**これが今日のリファクタリング対象です。** 2008年時代のテーブルレイアウト、`<font>` タグ、画像ベースのナビゲーション — 典型的なレガシーコードです。

## PART 1: セットアップ＆カスタマイズ
Duration: 20
...
Copilot がレガシーコードの以下を理解していることを確認してください：
- HTML 4.01 Transitional のテーブルレイアウト
- `<font>` タグによるスタイリング
- 画像ベースのナビゲーション（SVG）
- 固定幅 760px のデザイン
...
`copilot-instructions.md` を以下の内容に書き換えてください：
...
```
please make a research to create a frontend skill that follow the branding of Nikora Corporation (the company in 1.nikora-website/). 
...
Copilot がウェブサイトを分析し、ブランドガイドラインに基づいた Skill ファイルを自動生成します。
...
```
このレガシー HTML サイト（1.nikora-website/）を Next.js + TypeScript + Tailwind CSS に移行してください。
...
1.nikora-website/ の内容を分析し、新しい Next.js プロジェクトとして構築してください。
```
