...
```
小さな Astro サイトを作ってください（日本語のみ。i18n / 英語対応は不要）。
...
>
> ここで重要なポイント — **Copilot Chat に直接「ここを直して」と頼むのではなく、それぞれを GitHub Issue として登録します**。Part 11（CODE: CLI）で、これらの Issue を Copilot CLI に **まとめて並列に** 実装してもらうためです。
>
...
セクション 6 で Copilot Chat が作ったサイトはまだローカル（Codespace 上のブランチ）にあるだけです。これをリモートに上げて PR を作ると、7.2 で設定した Ruleset によって Copilot Code Review が自動的に走ります。

1. **VS Code のソース管理タブ** で変更をステージ → コミットメッセージを入力 → **Commit**
2. **Sync Changes** （または **Publish Branch**）でリモートに push
3. GitHub.com のリポジトリで **Compare & pull request** をクリックして PR を作成
4. PR を開くと、Copilot Code Review が自動的に実行され、以下が表示されます（⏱ 約5分）：
   - ✅ **Pull Request Overview** — PR 全体のサマリーコメント
   - ✅ **コード提案** — 各ファイルに対する具体的な改善提案

> aside positive
> **ポイント**: ブランチ名は何でも OK ですが、`feature/<内容>` のような形式が一般的です。Copilot Chat の Agent モードが自動でブランチを切っている場合はそのまま使えます。
...
### 11.3 — `/fleet` で Issue を並列に実装する

Part 6 で作成した 3〜5 件の Issue を、CLI に並列で実装させます。

<details>
<summary>📋 プロンプトを表示 / コピー</summary>

```
/fleet このリポジトリの open Issue 一覧を gh CLI で取得してください
（`gh issue list --state open --json number,title,labels`）。
その中から、Part 6 で作ったバグ修正・UI 改善系の Issue（label が bug または enhancement のもの）をすべて拾い、
1 Issue につき 1 つの fleet エージェントを割り当てて、並列で実装してください。

各エージェントは以下のフローで作業すること：
1. Issue 本文を読んで実装計画を立てる
2. Issue 番号と一致するブランチ (fix/<number>-<slug>) を切る
3. 修正を実装する
4. pnpm dev で動作確認できる状態にする
5. コミットして PR を出す（`Closes #<number>` を本文に入れる）

すべての Issue を処理し終えたら、作成された PR のリストを表示してください。
```
...
CLI が複数の fleet エージェントを spawn し、それぞれ別ブランチで実装 → PR 作成まで自律的に進めます。進捗はターミナルにストリーム表示されます。

### 11.4 — 結果をレビューする

実装が一通り終わったら、各 PR を CLI からレビューします：

```
/review すべての open PR を順番にレビューして、問題点と改善案を簡潔にまとめてください
```

### 11.5 — Chronicle で利用状況を分析する
