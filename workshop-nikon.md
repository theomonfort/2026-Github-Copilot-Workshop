...
### 12.4 — Codex モデルでラバーダックレビューする

実装が一通り終わったら、**別モデル（Codex 系）の視点** で fleet が生成した変更全体をラバーダックレビューします。同じモデルでレビューすると同じ盲点を見逃すので、モデルを切り替えるのがポイントです。

```
/model
```

モデル選択画面で **GPT-5 Codex** を選択してください。
...
```
fleet エージェントが作った open PR をすべて列挙して、それぞれの差分を読み、
ラバーダック視点でレビューしてください。観点は以下：

1. 設計上の盲点・ロジックバグ
2. Issue の要件を満たしているか（仕様の漏れ）
3. 既存コードとの整合性（命名・スタイル・パターン）
4. テストやエラーハンドリングの不足

スタイルや軽微な指摘は無視して、本当に重要な問題だけ簡潔に PR 単位で列挙してください。
```

</details>

> aside positive
> **ポイント**: 実装させたモデル（Claude）と異なるモデル（Codex）でレビューすることで、同質性バイアスを避けて盲点を炙り出せます。これが「ラバーダックを別のモデルにやらせる」発想です。

### 12.5 — Chronicle で利用状況を分析する
