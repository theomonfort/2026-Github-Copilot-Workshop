...
Copilot に以下のプロンプトを入力して、フロントエンド開発用の Skill を作成してもらいましょう。`[name of your company]` の部分を**自社の会社名に置き換えて**ください：

```
please make a research to create a frontend skill that follow the branding of [name of your company]. 
...
> aside negative
> **⚠️ 重要**: `[name of your company]` をそのまま入力しないでください。必ず自社の会社名（例: `Nikon`、`Sony`、`Toyota` など）に置き換えてください。
